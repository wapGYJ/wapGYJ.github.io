...
func RePassword(context *gin.Context) {
	old_password := context.Query("old_password")
	new_password := context.Query("new_password")
	fmt.Println("你的旧密码是", old_password)
	...
	_, err = DB.Exec("UPDATE tb_user SET password = ? WHERE username = ?", new_password, username)
	if err != nil {
		fmt.Println(err.Error())
		context.JSON(http.StatusInternalServerError, gin.H{"error": "密码更新失败"})
		return
	}
