...
func SearchBook(context *gin.Context) {
	BookName := context.Query("book_name")
	//链接数据库
	...
	var book Bookinfo
	err = utils.DB.QueryRow("select id,name,is_star,comment_num,score,cover,publish_time,link,author,lable from tb_book where name=?", BookName).Scan(&book.Id,
